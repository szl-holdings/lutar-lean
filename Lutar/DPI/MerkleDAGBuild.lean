...
Root cause: line ~132, `complete_tree_node_count_formula` called `positivity` on
the goal `B ^ (h + 1) ≥ 1`, i.e., `1 ≤ B ^ (h + 1)`. In Lean 4 + Mathlib with
`leanprover/lean4:v4.13.0`, the `positivity` tactic cannot prove `1 ≤ B^n`
(it proves `0 < B^n` or `0 ≤ B^n` only; the lower bound 1 requires `hB : 2 ≤ B`
which gives `0 < B`). Fix: replace `positivity` with
`exact Nat.one_le_pow _ _ (by omega)`.

Strategy: real proof (one-line tactic replacement).
Confidence: HIGH — `Nat.one_le_pow` is stable in Lean 4.13.0 / Mathlib.
...
/-- For a complete B-ary tree of height h, node count = (B^(h+1) - 1) / (B - 1).
    This lemma proves the simpler bound B^(h+1) ≥ 1.

    **Fix applied**: replaced `positivity` (which cannot prove `1 ≤ B^(h+1)`,
    only `0 ≤` or `0 <`) with `exact Nat.one_le_pow _ _ (by omega)`.
    `Nat.one_le_pow : ∀ (n : ℕ) (b : ℕ), 1 ≤ b → 1 ≤ b ^ n` — stable
    in Lean 4.13.0 / Mathlib (no API drift). -/
